## Observações
O sinal não foi capturado na execução. Pesquisando a documentação das funções
sleep e alarm no sistema operacional linux utilizado (Debian), encontramos na
seção de observações a seguinte mensagem:
...
Logo, o processo pai não consegue capturar o sinal do alarm, pois assim como o
sleep é tangivel apenas ao seu processo, o alarm também é (afinal os dois usam SIGALRM).
Logo o pause do processo pai vai ficar esperando um sinal que não foi lançado para ele.
